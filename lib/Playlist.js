...
export default class Playlist extends Map {
  ...
  leastPlayedTrack (stars, artists = new Set()) {
    const candidates = stars ? this.tracksWithStars(stars) : this.tracks.filter(track => track.stars > 2)
    if (candidates.length < 1) return null
    const songsByOtherArtists = candidates.filter(track => !artists.has(track.artist)).sort((a, b) => a.activity - b.activity || a.random - b.random)
    return ((songsByOtherArtists.length > 0) ? songsByOtherArtists : candidates.sort((a, b) => a.activity - b.activity || a.random - b.random))[0]
  }
  ...
  get summary () {
    return [
      `\t“${this.name}” (${this.id})`,
      `\t   ★★ · ${String(this.tracksWithStars(2).length).padStart(4)} tracks`,
      `\t  ★★★ · ${String(this.tracksWithStars(3).length).padStart(4)} tracks`,
      `\t ★★★★ · ${String(this.tracksWithStars(4).length).padStart(4)} tracks`,
      `\t★★★★★ · ${String(this.tracksWithStars(5).length).padStart(4)} tracks`,
      `\ttotal · ${String(this.length).padStart(4)} tracks`,
      `\tdistinct artists: ${this.artists.size}`,
    ].join('\n')
  }
